...
function checkNodeConn(node) {
  ...
  //send a get request and capture the response
    checkConnRequest = https.request(https_checkConn, function (res) {
      ...
      res.on("data", function (chunk) {
        ...
        resultString = "<br/>Execution Node at " + node.hostname + ":" + node.port + " working: " + body + "\n";
        console.log("nodeing");
        return resultString;
      });
    });
  ...
  checkConnRequest.on("error", function (e) {
    resultString = "<br/>Execution Node at  " + node.hostname + ":" + node.port + " Error: " + e.message + "\n";
    return resultString;
  });
...
app.get("/connectionCheck", function (req, res) {
  ...
  for (i = 0; i < nodes_data.Nodes.length; i = i + 1) {
    ...
    result_connectionCheck = result_connectionCheck.concat(checkNodeConn(nodes_data.Nodes[i]));
  }
...
setInterval(connectToSQL, 10000);
