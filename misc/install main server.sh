...
mysqlid=` sudo docker run \
--detach \
--env="MYSQL_ROOT_PASSWORD=root" \
--publish 3306:3306 \
--env="MYSQL_DATABASE=Autolab" \
...
sudo docker run --detach --hostname localhost --env GITLAB_ROOT_PASSWORD="sangoltejas" --publish 80:80 --publish 443:443 --name gitlab --restart always --volume /srv/gitlab/config:/etc/gitlab --volume /srv/gitlab/logs:/var/log/gitlab   --volume /srv/gitlab/data:/var/opt/gitlab gitlab/gitlab-ce:latest

    sudo docker run --name load_balancer --net=host -v /home/tejas/Desktop/submissions:/load_balancer/submissions -d -i -t load_balancer