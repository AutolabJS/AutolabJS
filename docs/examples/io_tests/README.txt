...
Then execute.sh tries to use "test_cases/<language>/setup/Test.sh" script to perform the test number 1. Test.sh script
in turn relies on compile.sh and executeTest.sh scripts to perform compile time and run time work.
...
Flexibility with the existing setup:
	...
	2) all of the testing strategy is delegated to specific-test scripts like Test.sh etc. If the testing strategy differs
	  from test to test, write appropriate test files for a language, say Test#.java and copying the right files using Test#.sh.
...
A lab author has to perform the following tasks.
	...
	2) Modify copy statements in Test.sh or create appropriate Test#.sh files to select the testing strategy.
	   If compile#.sh and executeTest#.sh have been written for a test case, corresponding lines need to be modified in Test#.sh.
		 If no customization is required, then leave the existing code alone.
